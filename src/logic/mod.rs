...
impl Game {
    ...
    pub fn run(&mut self) {
        ...
        let image = Image::load_image("resources/block/1.png").expect("Failed to load image");
        let texture = rl.load_texture_from_image(&thread, &image).expect("Failed to load texture");

        // 加载窗口图标
        let icon = Image::load_image("resources/block/1.png").expect("Failed to load icon");
